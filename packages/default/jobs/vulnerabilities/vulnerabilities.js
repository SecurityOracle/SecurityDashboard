...
module.exports = {
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/vulnerabilities file)
     ...
     Have a look at test/vulnerabilities for an example of how to unit tests this easily by mocking easyRequest calls
    ...
    dependencies.easyRequest.JSON("https://security-oracle.eu-gb.mybluemix.net/app/" + appId, function (err, reportIds) {
      // logger.trace(reports);
      
      async.map(reportIds, function(reportId, cb) {
          getReport(reportId, dependencies, cb);
      }, function(err, reports) {
          jobCallback(err, {title: config.widgetTitle, reports: reports});
      });
    });
  }
};
